...
const props = withDefaults(defineProps<Props>(), { value: '', defaultValue: '', format: 'hex' });

const inputColorRgb = ref<Rgba>({ r: 255, g: 255, b: 255, a: 1 });

const inputColorHex = ref<string>('FFFFFFFF');
...
watch(
  ...
  (color) => {
    ...
    inputColorHex.value = tinycolor(color).toHex8String().replace('#', '').toUpperCase();
  }
);
...
function handleInputHexBlur() {
  ...
  inputColorHex.value = color.toHex8String().replace('#', '').toUpperCase();
